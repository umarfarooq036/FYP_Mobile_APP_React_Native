...
export default function ShopPage({ route }) {
  ...
  const renderHours = () => {
    if (!openingHours) {
      return defaultHours; // Return default hours if openingHours is null or undefined
    }
  
    const { start, end } = openingHours;
    
    if (!start || !end) {
      return defaultHours; // Return default hours if start or end is null or undefined
    }
  
    return `Monday - Friday, ${start} - ${end}`; // Render opening hours if all values are present
  };
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.description}>
          ...
          <Text>
            {/* <Text style={styles.boldText}>Opening Hours:</Text> {shop?.openingHours ? `Monday - Friday, ${start} - ${end}` : "Monday - Friday, 9:00 AM - 6:00 PM"} */}
            <Text style={styles.boldText}>Opening Hours:</Text>
            {/* {renderHours()} */}
            time
          </Text>
          <Text>
